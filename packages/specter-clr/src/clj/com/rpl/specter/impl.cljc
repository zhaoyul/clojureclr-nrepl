...
#?(
   :clj
   (defn array-copy-of [arr new-len]
     (java.util.Arrays/copyOf arr (int new-len)))
   :cljr
   (defn array-copy-of [arr new-len]
     (let [n (int new-len)
           out (object-array n)
           max-len (alength arr)
           copy-len (if (< max-len n) max-len n)]
...
(extend-protocol CoercePath
  ...
  #?(:clj java.util.List :cljr clojure.lang.IPersistentVector :cljs cljs.core/PersistentVector)
  (coerce-path [this]
    (do-comp-paths this))
...
(defn do-compiled-traverse* [apath structure]
  (reify #?(:clj clojure.lang.IReduce :cljr clojure.lang.IReduce :cljs cljs.core/IReduce)
    (#?(:clj reduce :cljr reduce :cljs -reduce)
        [this afn]
      #?(:bb (reduce afn (afn) this)
         :clj (.reduce this afn (afn))
         :cljr (.reduce this afn (afn))
         :cljs (-reduce this afn (afn))))
    (#?(:clj reduce :cljr reduce :cljs -reduce)
      [this afn start]

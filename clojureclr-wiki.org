...
ClojureCLR 支持 clojure.org/java_interop 中提到的所有基础互操作函数。
...
#+BEGIN_SRC clojure
  ;; CLR 特有的数值类型提示
  ;; 注意：ClojureCLR 支持 uint/ulong 等扩展类型，但类型提示可能需要完全限定
  (defn process-uint [^System.UInt32 x]
    (bit-or x 0xFF))

  (defn process-ulong [^System.UInt64 x]
    (+ x 100))

  ;; 测试
  (process-uint (uint 100))
  ;; => 255

  (process-ulong (ulong 1000))
  ;; => 1100

  ;; 数组类型简写 - uints 创建 UInt32 数组
  (defn process-uint-array [arr]
    (alength arr))

  ;; 创建 uint 数组
  (def uint-arr (uints 1 2 3 4 5))
...
;; 指定具体类型参数
(def dict (|System.Collections.Generic.Dictionary`2[System.String,System.Int32]|.))
...
(deftype DualService []
  ICalculator
  (^Int32 calculate [this ^Int32 x]
    (* x 2))  ; ICalculator 的实现：乘以 2

  IProcessor
  (^Int32 user.IProcessor.calculate [this ^Int32 x]  ; 显式实现
    (+ x 10)))  ; IProcessor 的实现：加 10

;; 测试
(def service (DualService.))

;; 默认调用 ICalculator 的实现
(.calculate service 5)
;; => 10

;; 通过类型提示调用 IProcessor 的实现
(.calculate ^user.IProcessor service 5)
;; => 15
#+END_SRC
...
(gen-interface
  :name IValidator
  :methods [[validate [Object] Boolean]
            [getErrorMessage [] String]])

;; 使用生成的接口
(deftype StringValidator [^Int32 minLength]
  IValidator
  (validate [this ^String value]
...
(.ToString rect)
;; => "ResizableRectangle[10.0x5.0]"

;; 缩放
(scale rect 2.0)
(.ToString rect)
;; => "ResizableRectangle[20.0x10.0]"

;; 调整大小
(resize rect 100.0 50.0)
(.ToString rect)
;; => "ResizableRectangle[100.0x50.0]"
#+END_SRC

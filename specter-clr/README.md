...
dotnet build specter-clr/specter-clr.csproj
```

Notes:
- The build embeds `.clj` resources (not `.cljc`) for ClojureCLR compatibility.
- `tools/gen-cljr.clj` generates CLR-friendly `.clj` files from `.cljc`. Re-run it after editing any `.cljc` files under `specter-clr/src/clj`.
...
```bash
clojure -Sdeps '{:deps {org.clojure/tools.reader {:mvn/version "1.3.7"}}}' -e '(load-file "tools/gen-cljr.clj")'
dotnet pack specter-clr/specter-clr.csproj -c Release -o nupkgs
```
